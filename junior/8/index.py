...
def clean(text: str, lemmatizer: WordNetLemmatizer):
    """Clean text"""
    text = str(text)
...
def clear_data(source_path: str, target_path: str, n_jobs: int):
    ...
    data["cleaned_text"] = Parallel(n_jobs=n_jobs, backend="multiprocessing")(
        delayed(clean)(text, lemmatizer) for text in data["text"]
    )
